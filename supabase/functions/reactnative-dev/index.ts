...
import { handleUpdateReactNative, reactNativeDevBot } from "../utils/telegram/bot.ts";
...
reactNativeDevBot.on("callback_query:data", async (ctx) => {
  ...
  if (callbackData === "start_test") {
    try {
      resetProgress({username: ctx.callbackQuery.from.username || "", language: "reactnative"});
      const questionContext = {
        lesson_number: 1,
        subtopic: 1,
      };

      const questions = await getQuestion({ctx: questionContext, language: "reactnative"});
