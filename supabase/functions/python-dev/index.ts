...
import { handleUpdatePython, pythonDevBot } from "../utils/telegram/bot.ts";
...
pythonDevBot.on("message:text", async (ctx) => {
  ...
  const token = `${Deno.env.get("FLOWISE_AI_PYTHON_DEV")}`
...
pythonDevBot.on("callback_query:data", async (ctx) => {
  ...
  if (callbackData === "start_test") {
    try {
      resetProgress({username: ctx.callbackQuery.from.username || "", language: "python"});
      const questionContext = {
        lesson_number: 1,
        subtopic: 1,
      };

      const questions = await getQuestion({ctx: questionContext, language: "python"});
...
Deno.serve(async (req) => {
  try {
    ...
    const result = await handleUpdatePython(req);
