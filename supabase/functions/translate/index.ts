...
serve(async (req: Request) => {
  ...
  for (const row of dataToTranslate) {
    const { question, variant_0, variant_1, variant_2, topic, question_en: defaultEn } = row;
    if (defaultEn === null) {
  ...
  return new Response('Translation and update completed successfully', { status: 200 });
});
